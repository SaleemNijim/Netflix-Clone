...
export default function UnauthPage() {
    ...
    function UnauthBanner({ router }) {
        return <div className="h-[65vh] sm:h-[90vh] xl:h-[95vh] bg-cover bg-no-repeat bg-[url('https://assets.nflxext.com/ffe/siteui/vlv3/84526d58-475e-4e6f-9c81-d2d78ddce803/e3b08071-f218-4dab-99a2-80315f0922cd/LK-en-20221228-popsignuptwoweeks-perspective_alpha_website_small.jpg')] border-b-8 border-gray-800  ">
        ...
        >
            <main>
                <div className="bg-[#000000]">
                    ...
                    <div className="border-b-8 border-gray-800 pd-8">
                        <div className="flex flex-col h-[85vh] lg:h-[95vh] text-white px-8 sm:px-28 md:px-48 xl:px-80 mt-3 sm:mt-14">
                            <h1 className=" mb-5 text-xl sm:text-3xl md:text-5xl lg:text-5xl text-bold text-center px-14 md:px-0">Frequently asked questions</h1>
                            {
                                questions.map((item, index) => (
                                    <div key={index} className="flex flex-col gap-3">

                                        <div
                                            onClick={() => setshowCurrentAns(showCurrentAns === index)}
                                            key={item.ans}
                                            className="felx justify-between p-3 lg:p-5 mt-2 bg=[#303030] cursor-pointer">
                                            <h2>{item.ques}</h2>
                                            <PlusIcon className="h-7 w-7" color="white" />

                                        </div>
                                        {
                                            showCurrentAns === index && <div
                                                className="p-3 lg:p-5 mt-2 bg=[#303030] cursor-pointer">{item.ans}</div>
                                        }
                                    </div>)
                                )
                            }
